  ...
  test('MonitorOwner: GET /monitor/{monitorGroupScalingId} (Получение группового монитора Scaling)', async () => {
    ...
    await request
      ...
      .then(({ body }: { body: MonitorGetResponse }) => {
        ...
        expect(body.data.groupIds?.[0]?.row).toBeTruthy();
        expect(body.data.groupIds?.[0]?.col).toBeTruthy();
        expect(body.data.groupIds?.[1]?.monitorId).toBeTruthy();
        expect(body.data.groupIds?.[1]?.row).toBeTruthy();
        expect(body.data.groupIds?.[1]?.col).toBeTruthy();
        expect(body.data.groupIds?.[2]?.monitorId).toBeTruthy();
        expect(body.data.groupIds?.[2]?.row).toBeTruthy();
        expect(body.data.groupIds?.[2]?.col).toBeTruthy();
        expect(body.data.groupIds?.[3]?.monitorId).toBeTruthy();
        expect(body.data.groupIds?.[3]?.row).toBeTruthy();
        expect(body.data.groupIds?.[3]?.col).toBeTruthy();
        expect(body.data.user?.password).toBeUndefined();
      });
  });
